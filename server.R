...
function(input, output) {
  ...
  output$correspondencias <- renderPlot({
    ...
    mapacor <- ggplot(grafico, aes(x, y, label=row.names(grafico))) +
      ...
      ylab(paste0("Dimension 2: ",porcy,"%")) +
      xlab(paste0("Dimension 1: ",porcx,"%")) +
  ...
  output$downloadCA <- downloadHandler(
    filename = function() { paste("sampledata", '.xlsx', sep='') },
    content = function(file) {
      datos <- datglobal$datosCA
      write.xlsx(datos, file, row.names=T)
    }
  )
